...
function run_pytest() {
  ...
  if ! python3 -m pytest "${project_root_dir}" "$@" --cov=cli_exit_tools; then
...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : $HOME/venv/bin/cli_exit_tools --version"
  if ! "$HOME/venv/bin/cli_exit_tools" --version; then
